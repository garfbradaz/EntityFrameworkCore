...
namespace Microsoft.EntityFrameworkCore.Query.Internal
{
    ...
    {
        private static readonly ISet<MethodInfo> _supportedMethods = new HashSet<MethodInfo>
            {
        ...
        {
            if (methodCallExpression.TryGetEFPropertyArguments(out var source, out var _))
            {
                source = Visit(source);
            ...
            MethodCallExpression methodCallExpression, Func<Expression, bool, Expression> createSelector, Type returnType)
        {
            ...
            if (!_predicateLessMethodInfo.TryGetValue(methodCallExpression.Method.GetGenericMethodDefinition(), out var methodInfo))
            {
                methodInfo = methodCallExpression.Method.GetGenericMethodDefinition();
            }

            source = Expression.Call(methodInfo.MakeGenericMethod(source.Type.TryGetSequenceType()), source);
